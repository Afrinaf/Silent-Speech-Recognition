...
if __name__ == '__main__' :
    ...
    train_filenames = [s for s in train_filenames if int(s.split('\\')[2].split('_')[0]) < params["num_classes"]]
    #print (train_filenames)
    train_labels = [int(s.split("\\")[2].split('_')[0]) for s in train_filenames]
    
    # Validation data
    val_pathlist = Path(val_dir).glob("*.jpg")
    val_filenames = [str(path) for path in val_pathlist]
    val_filenames = [s for s in val_filenames if int(s.split('\\')[2].split('_')[0]) < params["num_classes"]]
    
    val_labels = [int(s.split("\\")[2].split('_')[0]) for s in val_filenames]
